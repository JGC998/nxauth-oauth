...
import { adminDeleteAsignatura } from '@/lib/actions';
import { useRouter } from 'next/navigation';
import { useEffect } from 'react';

function DeleteButton({ asignaturaId }) {
    const [state, action, isPending] = useActionState(adminDeleteAsignatura, undefined);
...
export default function AsignaturaList({ asignaturas }) {
                        ...
                        <tr key={asignatura.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-500">
                                    {asignatura.profesor || <span className="text-gray-400">Sin asignar</span>}
                                </div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                    {asignatura.horas_semana}h
                                </span>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <DeleteButton asignaturaId={asignatura.id} />
                            </td>
                        </tr>
                    ))}
                </tbody>
